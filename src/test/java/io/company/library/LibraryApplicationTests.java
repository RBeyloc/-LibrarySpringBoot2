...
class LibraryApplicationTests {
	...
	void createBooks(){
		...
		int initialCount = bookService.count();
		...
		int finalCount = bookService.count();

		assertEquals(initialCount + 4, finalCount);
	...
	void createAuthors(){
		...
		int initialCount = authorService.count();
		...
		Author author3 = new Author("Emile", "Zola", date2 );
		...
		int finalCount = authorService.count();
	...
	void assignAuthorToNewBooks(){
		...
		Optional<Author> authorFound = authorService.findAuthorById(47L);
	...
	void assignNewBooksToAuthor(){
		//find by id just one author
		Optional<Author> authorFound = authorService.findAuthorById(1010L);
		//if author exists, then..
		if(authorFound.isPresent()) {
			//create books
			Book book1 = new Book("War and Peace",  462, 1867, "SSSGV-5-ERG5-6567");
